...
system_tools.py         ← System Tools 插件 (20 tools: fs/shell/web/task/process/system/skill)
...
提供 20 個系統工具，涵蓋檔案系統、shell、網路、任務管理、進程管理和系統資訊：
...
| 工具 | 功能 | 關鍵參數 |
|------|------|---------|
| `web_fetch` | HTTP GET 抓取 URL 內容（urllib） | `url`, `timeout`（預設 15s）, `max_size`（預設 500KB） |

#### 後台任務管理（7 個）

後台任務 = shell 命令的非同步執行。每個任務有 task_id、name、status（running/completed/stopped）。

| 工具 | 功能 | 關鍵參數 |
|------|------|---------|
| `task_create` | 啟動後台任務 | `command`, `name` |
| `task_get` | 取得任務資訊 | `task_id` |
| `task_list` | 列出所有任務 | — |
| `task_update` | 更新任務 metadata（如 name） | `task_id`, `name` |
| `task_stop` | 停止執行中的任務 | `task_id` |
| `task_output` | 取得任務的 stdout/stderr | `task_id` |
| `task_delete` | 刪除任務及其輸出 | `task_id` |

#### 進程管理（4 個）

進程管理 = 長執行子進程的生命週期控制。與 task 的差異：process 以 PID 為索引，更接近 OS 層級。

| 工具 | 功能 | 關鍵參數 |
|------|------|---------|
| `process_spawn` | 啟動子進程（支援 setsid） | `command`, `name` |
| `process_kill` | 終止子進程 | `pid` |
| `process_list` | 列出所有追蹤的子進程 | — |
| `process_status` | 查詢子進程狀態 | `pid` |

#### 系統資訊（1 個）

| 工具 | 功能 | 關鍵參數 |
|------|------|---------|
| `system_info` | 回傳 OS/CPU/memory/Python 版本/cwd 等 | — |

#### Skill 執行（1 個）

| 工具 | 功能 | 關鍵參數 |
|------|------|---------|
| `execute_skill` | 載入 SKILL.md 的 body 內容 | `skill_name` |

設計原則：
- 全部用 Python stdlib（pathlib, subprocess, re, glob, urllib），零外部依賴
- bash 輸出上限 100KB，web_fetch 上限 500KB，超過截斷
- grep 最多回傳 500 個 match，掃描最多 1000 個檔案
- 錯誤一律回傳 `Error: ...` 字串（不拋異常），讓 LLM 能讀懂
- 任務和進程的狀態存在 plugin 實例記憶體中（thread-safe）
...
|------|------|
| `Ctrl+Q` | 退出程式 |
| `Ctrl+S` | 進入/退出選取模式（TextArea，可滑鼠反白 + Ctrl+C 複製） |
| `Esc` | 退出選取模式 |
| `Ctrl+D` | 切換 system context detail |
| `Ctrl+T` | 切換 tools detail（顯示 [✓] active / [·] deferred） |
| `Ctrl+B` | 切換 msgs detail |
| `Ctrl+L` | 清除對話面板 |
| `↑` / `↓` | 輸入框中循環前 5 條歷史命令 |
...
`Ctrl+S` 切換選取模式：隱藏 RichLog，顯示 `TextArea(read_only=True)` 載入完整純文字對話記錄（user + thought + tool call + result + answer）。支援滑鼠反白選取 + `Ctrl+C` 複製。`Esc` 或再按 `Ctrl+S` 回到正常介面。
...
| `on_thought` | 💭 thought → RichLog + plain_log |
| `before_action` | 🔧 tool name + input → RichLog + plain_log |
| `on_observation` | 📋 tool result → RichLog + plain_log |
| `on_token_usage` | status bar + context usage（動態 tool tokens） + context_content |
| `on_agent_end` | ✓ 最終答案 → RichLog + plain_log |
...
MODEL_CONTEXT_WINDOWS = {
    ...
    "claude-opus-4-6-20250724": 1_000_000,  # 1M context
    ...
}
```

- Context panel threshold 自動從 worker model 查表（200k 或 1M）
- 壓縮門檻 = context window × `threshold_pct`（預設 50%）
- CLI：`--threshold-pct 30` → 200k × 30% = 60k 壓縮門檻
...
def create_agent(
    ...
    threshold=100000,                            # 預設 context window 50%
...
`create_agent()` 自動註冊以下插件：
- **EpisodeCuratorPlugin** — Episode 管理 + recall_episode 工具
- **SystemToolsPlugin** — 20 個系統工具（deferred，需 load_tools 載入）
- **SkillLoaderPlugin** — 掃描 `skills/` 目錄，將 skill catalog 注入 system prompt
- **ToolRegistryPlugin** — 動態工具載入/卸載（load_tools/unload_tools）
...
25. **web_fetch**：HTTP GET 抓取 URL、timeout、max_size 截斷、錯誤處理
26. **後台任務管理**：task_create/get/list/update/stop/output/delete 完整生命週期
27. **進程管理**：process_spawn/kill/list/status、setsid 隔離、kill 後狀態正確
28. **系統資訊**：system_info 回傳 OS/CPU/memory/Python/cwd 等 JSON
29. **Skill 執行**：execute_skill 載入 SKILL.md body、frontmatter 剝離、不存在的 skill 報錯
30. **create_agent 完整註冊**：工廠函式自動註冊 SystemToolsPlugin + ToolRegistryPlugin
...
| 12 | System Tools (20 tools: fs/shell/web/task/process/system/skill) | DONE | 66 | — |
...
| 25 | web_fetch HTTP GET | `test_system_tools_extended.py::TestWebFetchTool` | PASS |
| 26 | 後台任務管理 (task_*) | `test_system_tools_extended.py::TestTaskManagement` | PASS |
| 27 | 進程管理 (process_*) | `test_system_tools_extended.py::TestProcessManagement` | PASS |
| 28 | 系統資訊 (system_info) | `test_system_tools_extended.py::TestSystemInfo` | PASS |
| 29 | Skill 執行 (execute_skill) | `test_system_tools_extended.py::TestExecuteSkill` | PASS |
| 30 | create_agent 註冊 SystemToolsPlugin | `test_system_tools_extended.py::TestCreateAgentRegistration` | PASS |
| 31 | load/unload 在 tools panel 可見性 | `test_load_unload_visibility.py` | PASS |
| 32 | tool tokens 動態反映 active set | `test_tool_tokens_and_skill_injection.py::TestToolTokenEstimation` | PASS |
| 33 | SkillLoader 註冊 + skill catalog 注入 | `test_tool_tokens_and_skill_injection.py::TestSkillLoaderRegistration` | PASS |
| 34 | Skill close-book 壓縮/re-expand | `test_context_panel_redesign.py::TestSkillCloseBook` | PASS |
| 35 | msgs panel 顯示 tool_result 內容 | `test_context_panel_redesign.py::TestMsgsPanelToolResultVisibility` | PASS |
| 36 | Context panel 堆疊進度條 (% of threshold) | `test_context_panel_redesign.py::TestContextUsagePanelRedesign` | PASS |
| 37 | Skill 路徑解析 (absolute path) | `test_context_fixes.py::TestSkillPathResolution` | PASS |
| 38 | Context threshold = model window (200k) | `test_context_fixes.py::TestContextThresholdIsModelWindow` | PASS |
| 39 | Context panel 高度顯示完整 | `test_context_fixes.py::TestContextPanelHeight` | PASS |
| 40 | 模型 context window 映射 (200k/1M) | `test_model_context_config.py::TestModelContextWindows` | PASS |
| 41 | 壓縮門檻 = context window × pct (預設 50%) | `test_model_context_config.py::TestCompressionThresholdDefault` | PASS |
| 42 | CLI --threshold-pct 參數 | `test_model_context_config.py::TestCLIThresholdPct` | PASS |
| 43 | Context panel compress threshold 顯示 | `test_ui_enhancements.py::TestThresholdDisplay` | PASS |
| 44 | web_fetch 大小限制 (50KB) | `test_ui_enhancements.py::TestWebFetchSizeCap` | PASS |
| 45 | 輸入歷史 ↑↓ (max 5) | `test_ui_enhancements.py::TestInputHistory` | PASS |
| 46 | 選取模式 Ctrl+S (TextArea) | `test_select_copy.py::TestSelectionMode` | PASS |
